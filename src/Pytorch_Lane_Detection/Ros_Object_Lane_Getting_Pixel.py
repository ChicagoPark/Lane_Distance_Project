...
def callback_img2(img2):
    ...
    for i in range(num_lane):
        ...
        if len(leftx) != 0 and len(lefty) != 0:
            ...
            ploty = np.linspace(int(frame2.shape[0]*(1/2)), frame2.shape[0] - 1,
                                int(frame2.shape[0] * (1 / 2)))  # y좌표에 대해서 상위 1/3 지점 부터 최하단 까지 그래프를 그리기위한 코드
            left_fitx = left_fit[0] * ploty ** 2 + left_fit[1] * ploty + left_fit[2]  # 방정식에 대한 x 값을 얻어오기 위한 코드
            for ii, x in enumerate(left_fitx):
                q = (x, ii + int(frame2.shape[0]* (1/2)))  # 상위 1/3 지점부터 그래프를 그리기 시작하므로 인덱스에 해당하는 ii 에 그만큼 추가해준다.
                if (x >= 0):
                    k.append(q)
